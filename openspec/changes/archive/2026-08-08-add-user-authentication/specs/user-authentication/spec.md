...
登录 SHALL 使用密码学安全随机源生成高熵 opaque bearer token，客户端获得 raw token，SQLite MUST 只保存其 64 位十六进制 SHA-256 hash。session MUST 保存 `createdAt`、`lastSeenAt` 与可空 `revokedAt`；成功鉴权 SHALL 更新 `lastSeenAt`，登出 SHALL 撤销当前 session。当前版本 MUST NOT 声称或实现自动过期策略。
...
前端 SHALL 提供直接消费共享 Auth DTO 的 typed authClient 与 Pinia auth state。bearer token MUST 是 localStorage 中唯一认证凭据；用户、密码、完整响应和服务端 session MUST NOT 写入 localStorage。存在 token 时 `initialize()` SHALL 调用 `/auth/me`；凭据失效或 logout SHALL 清除 token、用户和本地受保护 store，但 MUST NOT 删除服务端业务数据。
