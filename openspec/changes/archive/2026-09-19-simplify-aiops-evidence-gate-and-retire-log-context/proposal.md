...
- **删除"报告必须引用全部支撑证据"的隐藏门槛**（实施期由真实 smoke 发现）：报告节点的 claims 引用检查原本要求穷尽引用每一条支撑证据，漏一条就把信任状态降级。实测一份基于 **8 条真实命中**的报告因为只引用了 **6 条**被判证据不足。规格只要求"每个关键结论建立 report_evidence_link"，并没有要求穷尽引用；这条检查比规格更严，而且会误伤重复命中与和结论无关的背景日志。
