...
| 参数种类 | 归属 | 理由 |
|---|---|---|
| `Region` / `TopicId` | **服务端无条件注入**，且从模型可见说明中移除 | 账号级权威配置，模型不该知道也不该覆盖 |
| **计划保证的跨步配对**（SearchLog 的 `Query` ← query-builder 产出；DescribeLogContext 的 `Time`/`PkgId`/`PkgLogId` ← 最近一次日志命中） | **执行者确定性绑定**，模型不填也看不到 | 这些是"搬运"而不是"判断"；交给模型会让"计划必须含 query-builder"这条校验失去意义，也会把已验证的 CQL 换成模型复述的版本 |
| 其余需要判断的字段（时间窗、`Limit`、`Text`、其他工具自有参数） | **模型填写** | "取哪个值、怎么描述检索意图"正是它该判断的事 |

**实现期修正**：本决策最初写的是"跨步参数一律模型填写"，落地时发现它与"计划强制 query-builder"自相矛盾——那个步骤的产出会被模型重写覆盖，等于白做。据此调整为上表的三分法，delta spec 同步修正。
